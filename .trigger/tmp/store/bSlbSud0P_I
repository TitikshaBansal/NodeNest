...
// src/trigger/tasks/extractFrame.ts
init_esm();
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      if (typeof timestamp === "string" && timestamp.endsWith("%")) {
        const durationCommand = `ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${tempInputPath}"`;
      ...
      const ffmpegCommand = `ffmpeg -i "${tempInputPath}" -ss ${timestampSeconds} -vframes 1 "${tempOutputPath}" -y`;
...
//# sourceMappingURL=chunk-5PWG5ZSV.mjs.map
